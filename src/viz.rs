    ...
    HttpServer::new(move || {
        App::new()
            ...
            .service(web::scope("tiles")
                ...
                .service(web::resource("coverage/{z}/{x}/{y}")
                    .route(web::get().to(mvt_coverage_get))
                )
                .service(web::resource("network/{z}/{x}/{y}")
                    .route(web::get().to(mvt_network_get))
                )
            )
            .service(
...
fn mvt_network_get(
    ...
    let tile = match mvt::network(&db, &iso.0, z, x, y) {
        Ok(tile) => tile,
        Err(err) => {
            println!("{}", err);
